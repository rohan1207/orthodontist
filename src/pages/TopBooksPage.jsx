...
function FlipBookCard({ book }) {
	...
	return (
		<motion.div
			...
			style={{ perspective: '2000px', aspectRatio: '3/4', containIntrinsicSize: '300px 400px' }}
			initial={false}
		...
		>
			<div
				...
				style={{ transformStyle: 'preserve-3d', transformOrigin: 'left', willChange: 'transform', isolation: 'isolate' }}
			>
				{/* Book Spine */}
				<motion.div
					className="absolute left-0 top-0 bottom-0 w-[20px] bg-gradient-to-r from-gray-800 to-gray-700"
					style={{ transformOrigin: 'left', rotateY: '-90deg', translateX: '10px' }}
				/>

				{/* Right Pages Stack */}
				<div
					className="absolute right-0 top-0 bottom-0 w-[15px] bg-white shadow-inner transform-gpu"
					style={{ transformOrigin: 'right', transform: 'rotateY(-20deg) translateX(15px)', background: 'linear-gradient(to left, #e5e7eb 0%, #fff 100%)' }}
				/>

				{/* Inner Pages */}
				<motion.div
					className="absolute w-full h-full bg-white rounded-r-lg overflow-hidden"
					variants={{ open: { opacity: 1, transition: { duration: 0.4, delay: 0.2 } }, closed: { opacity: 0, transition: { duration: 0.2 } } }}
				>
					<div className="h-full flex flex-col p-5 md:p-6 bg-gradient-to-r from-white to-gray-50">
						<div className="flex-1 min-h-0">
							...
							<p className="text-gray-600 text-sm leading-relaxed line-clamp-4">{book.description}</p>
				...
				<motion.div
					...
					variants={{
						open: { rotateY: -170, transition: { type: 'spring', stiffness: 50, damping: 15, restDelta: 0.5 } },
						closed: { rotateY: 0, transition: { type: 'spring', stiffness: 60, damping: 15 } },
					}}
				>
					{/* Front Cover */}
					<motion.div className="absolute inset-0 w-full h-full" style={{ backfaceVisibility: 'hidden', transform: 'translateZ(1px)' }}>
						<SafeImage src={book.coverImage} alt={book.title} className="w-full h-full object-cover rounded-lg" />
						<motion.div
							className="absolute inset-0 rounded-lg"
							style={{ background: 'linear-gradient(to right, rgba(0,0,0,0.2) 0%, rgba(0,0,0,0) 20%)', pointerEvents: 'none' }}
					...
					<motion.div
						...
						style={{ backfaceVisibility: 'hidden', transform: 'rotateY(180deg)', borderLeft: '1px solid rgba(0,0,0,0.1)' }}
					/>
				</motion.div>
			</div>
			...
			<motion.div
				...
				style={{ background: 'radial-gradient(circle, rgba(0,0,0,0.1) 0%, rgba(0,0,0,0) 70%)', width: '100%', height: '20px', transform: 'rotateX(60deg)' }}
				variants={{ open: { width: '90%', opacity: 0.3 }, closed: { width: '100%', opacity: 0.2 } }}
			/>
		</motion.div>
	);
}
				...
				<div className="p-5 flex flex-col">
					...
					<div className="mt-4 flex gap-2">
						...
						{book.ebookLink ? (
							...
							<Link to={`/book-summary/${book.id}`} className="inline-flex items-center gap-2 text-green-600 hover:text-green-700 font-medium">
