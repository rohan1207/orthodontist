		...
		>
			<div className="relative bg-white rounded-2xl p-6 shadow-lg border border-gray-100 h-full">
				<div className="flex items-start justify-between mb-4">
					<div className={`p-3 rounded-xl bg-gradient-to-r ${t.color}`}>
						<DocumentDuplicateIcon className="w-6 h-6 text-white" />
					</div>
					<div className="text-sm text-gray-500">{t.readTimeMin} mins</div>
				</div>
				<h3 className="text-lg font-semibold text-gray-900">{t.title}</h3>
				<p className="text-sm text-gray-500 mt-1">{t.sources.length} textbook sources</p>

				<div className="grid grid-cols-3 gap-4 my-6">
					<div className="text-center">
						<div className="text-sm text-gray-500">Key Points</div>
						<div className="font-semibold text-gray-700">{t.keyPoints}</div>
					</div>
					<div className="text-center border-x border-gray-100">
						<div className="text-sm text-gray-500">Read Time</div>
						<div className="font-semibold text-gray-700">{t.readTimeMin} min</div>
					</div>
					<div className="text-center">
						<div className="text-sm text-gray-500">Level</div>
						<div className="font-semibold text-gray-700">{t.difficulty}</div>
					</div>
				</div>

				<Link
					to={`/summaries/${t.id}`}
					className={`group w-full inline-flex items-center justify-center px-6 py-3 text-sm font-medium rounded-xl text-white bg-gradient-to-r ${t.color}`}
				>
					Read Full Summary
		...
		>
			<div className="flex flex-col md:flex-row md:items-center md:justify-between gap-5">
				<div className="flex items-start gap-4">
					...
					<div>
						<h3 className="text-lg font-semibold text-gray-900">{t.title}</h3>
						<p className="text-sm text-gray-500">{t.sources.length} textbook sources • {t.readTimeMin} min • {t.difficulty}</p>
						<div className="mt-3 grid grid-cols-3 gap-4 text-sm text-gray-600">
							<div className="inline-flex items-center gap-1"><BookOpenIcon className="w-4 h-4" /> {t.sources[0]}</div>
							<div className="inline-flex items-center gap-1"><ChartBarIcon className="w-4 h-4" /> {t.keyPoints} key points</div>
							<div className="inline-flex items-center gap-1"><ClockIcon className="w-4 h-4" /> {t.readTimeMin} min</div>
						</div>
					</div>
				</div>
				<Link to={`/summaries/${t.id}`} className="inline-flex items-center gap-2 text-green-600 hover:text-green-700 font-medium">
		...
		<div className="py-16 md:py-24 bg-gray-50/50">
			<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
				<div className="text-center mb-10 md:mb-14">
					<h1 className="text-3xl md:text-4xl font-medium text-gray-900 mb-3">All Topic Summaries</h1>
